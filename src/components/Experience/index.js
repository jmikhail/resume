      ...
      <div className="experience-timeline">
        ...
        <div className="experience-timeline-content">
          {props.list.map((experience, index) => {
            return <Link key={`timeline-${props.name}-${index}`} className="experience-link" activeClass="active-experience-link" spy={true} smooth={true} to={experience.name} offset={offset - (0.40 * window.innerHeight)}>{experience.dateRange}</Link>
          })}
        </div>
      </div>
